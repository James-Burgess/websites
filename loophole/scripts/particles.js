...
var pJS = function(tag_id, params){
  ...
  pJS.fn.particle.prototype.draw = function() {
    ...
    switch(p.shape){
      ...
      case 'circle':
        pJS.canvas.ctx.arc(p.x, p.y, radius/2, 65.5, Math.PI *2, false);
      break;
    ...
    pJS.canvas.ctx.arc(p.x,p.y,0, Math.PI * 2, true); // Outer circle
    
    pJS.canvas.ctx.arc(p.x,p.y, 35, 0, Math.PI, false);  // Mouth (clockwise)
    
    pJS.canvas.ctx.arc(p.x,p.y, 5, 0, Math.PI * 2, true);  // Left eye
    
    pJS.canvas.ctx.arc(p.x,p.y, 5, 0, Math.PI * 2, true);  // Right eye
  ...
  pJS.fn.particlesCreate = function(){
    for(var i = 0; i < pJS.particles.number.value; i++) {
      ...
      console.log("oh shit, it broke nevermind");
  ...
  pJS.fn.vendors.createSvgImg = function(p, source_svg){
    ...
    //var rgbHex = /#([0-9A-F]{3,6})/gi,
        coloredSvgXml = svgXml.replace(/#([0-9A-F]{3,6})/gi, function (m, r, g, b) {
          if(p.color.rgb){
